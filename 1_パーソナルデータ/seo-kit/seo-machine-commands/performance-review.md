...
`Report Generation` の結果から、クライアント共有用の短文サマリーを追加で作成する。  
**固定文構造を維持し、可変値のみ分析結果で差し替えること。**
...
方針：[低CTR要因 / 順位余地 / 検索意図に基づく1文]
```

**可変値の作成ルール**:
- `{対象記事数}`: 優先順位付け対象の総記事数
- `{期待クリック増分}`: 優先度キューの期待増分合計
- `{増加率}`: `(期待クリック増分 / 現状クリック合計) × 100`
- `方針`: Priority Queue と Detailed Analysis の根拠を1文で要約

**フォールバック**:
- 期待クリック増分を直接算出できない場合は、「主要KPI改善見込み」表現に切り替える

**出力ルール（必須）**:
- Markdown保存: `research/performance-review-client-brief-YYYY-MM-DD.md`
- ターミナル表示: 同内容をプレーンテキストで表示
- 共有文と本体レポートの数値不一致を許容しない
