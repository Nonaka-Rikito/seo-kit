...
CONTACT_KEYWORDS = [
    "contact", "inquiry", "inquire", "inquiries",
    "toiawase", "otoiawase", "form", "consulting",
    "support", "ask", "mail", "entry"
]

# URL-encoded Japanese keywords
CONTACT_KEYWORDS_ENCODED = [
    "%E3%81%8A%E5%95%8F%E3%81%84%E5%90%88%E3%82%8F%E3%81%9B",  # お問い合わせ
    "%E7%9B%B8%E8%AB%87",  # 相談
]
...
def is_external_domain(url, domain):
    ...
    if domain in url_host or url_host.endswith("." + domain):
        return False, url_host
    # ドメインのベース部分で比較
    domain_parts = domain.split(".")
    host_parts = url_host.split(".")
    # 2ndレベルドメインが一致するか
    if len(domain_parts) >= 2 and len(host_parts) >= 2:
        if domain_parts[-2] == host_parts[-2]:
            return False, url_host
    # 許容フォームサービスか
...
def has_contact_keyword(url):
    ...
    for kw in CONTACT_KEYWORDS:
        if kw in url_lower:
            return True
    for kw_enc in CONTACT_KEYWORDS_ENCODED:
        if kw_enc.lower() in url_lower:
            return True
    # Also check decoded URL
    try:
        from urllib.parse import unquote
        decoded = unquote(url_lower)
        jp_keywords = ["お問い合わせ", "相談", "問い合わせ", "問合せ"]
...
for i, row in enumerate(target_rows):
    row_num = 332 + i  # ヘッダー=1行目, データ1行目=2行目, ... 331番目のデータ行=332行目
    if len(row) < 3:
        continue
    domain = row[0].strip()
    company = row[1].strip()
    contact_url = row[2].strip()
    ...
    if contact_url:
        ...
        if is_ext:
            ...
            if domain and url_host_check and domain not in url_host_check and not url_host_check.endswith("." + domain):
                # Check if it's a form service
                is_form = False
                for allowed in ALLOWED_FORM_DOMAINS:
                    if allowed in url_host_check:
                        is_form = True
                        break
                if is_form:
                    summary["外部フォームサービス件数"] += 1

    # 3. contact系キーワードチェック（トップページ以外）
